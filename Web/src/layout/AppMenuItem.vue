...
<template>
    <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActive }">
        ...
        <a v-if="(!item.to || item.items) && item.visible !== false" href="javascript:void(0)" @click="itemClick($event, item)" :class="item.class" :target="item.target" tabindex="0" @mouseenter="onMouseEnter">
