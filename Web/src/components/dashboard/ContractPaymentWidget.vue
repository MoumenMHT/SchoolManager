...
<template>
  <div class="card">
    <div class="flex items-center justify-between mb-6">
      <h5 class="text-xl font-semibold">Contract Payment Status</h5>
    ...
    <DataTable
      ...
      :rowHover="true"
    >
      <Column field="contract_number" header="Contract #" style="min-width: 130px">
        <template #body="{ data }">
          <span class="font-mono text-sm font-semibold text-primary">{{ data.contract_number }}</span>
        </template>
      </Column>

      <Column field="parent_name" header="Parent" style="min-width: 150px">
        <template #body="{ data }">
      ...
      <Column field="paid_amount" header="Paid" style="min-width: 120px">
        <template #body="{ data }">
          <span class="font-semibold text-green-600 dark:text-green-400">{{ formatCurrency(data.paid_amount) }}</span>
        </template>
      </Column>

      <Column field="remaining_amount" header="Remaining" style="min-width: 130px">
        <template #body="{ data }">
          <span
            class="font-semibold"
            :class="data.remaining_amount > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'"
          >
            {{ data.remaining_amount > 0 ? formatCurrency(data.remaining_amount) : 'Settled' }}
          </span>
        </template>
      </Column>

      <Column field="payment_completion" header="Progress" style="min-width: 180px">
        <template #body="{ data }">
    ...
    <div v-else class="text-center py-8">
      ...
      <p class="text-muted-color">No active contracts found</p>
    </div>
  </div>
</template>
